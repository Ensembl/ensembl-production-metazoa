...
| Option | Example | Type: possible values | Action | Comment|
| - | - | - | -  | - |
`DATA_INIT` | `zgrep -v 'ID=idontlike' some.data.raw.asm.gff.gz > patched.gff`| `shell command`| Changes to `data/raw/asm` dir (see `ASM_URL` and `AMS_SINLGE` above) and runs specified commands; can be used multiple times; you can use abs path for files as well.
